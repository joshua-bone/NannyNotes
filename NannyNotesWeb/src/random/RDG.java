...
public class RDG {
	...
	public static void generateHouseholds(HouseholdDAO householdDAO, UserDAO userDAO, ChildDAO childDAO) {

		EntityManagerFactory emf = Persistence.createEntityManagerFactory("nannynotesdb");
		;
		EntityManager em = emf.createEntityManager();
		...
		try (BufferedReader fIn = new BufferedReader(
				new FileReader("/Users/joshuabone/SD/Java/workspace/RandomDataGenerator/firstnames.txt"))) {
			String line;
			while ((line = fIn.readLine()) != null) {
				firstNames.add(line);
			}
	...
	public static String householdNannyNote(ArrayList<String> names){
		...
		return "I've been having trouble with " + names.get(random.nextInt(names.size())) + " " + v + "ing with the " + rAdj() + " " + rNoun(true) + " lately.";
	}
	
	public static String householdParentNote(ArrayList<String> names){
		return "Please remember that " + names.get(random.nextInt(names.size())) + " has " + rNoun(false) + " practice this week.";
	}
	
	public static String nannyNote(String name){
		return name + " tends to " + rVerb() + " " + rAdv() + " but has been " + rAdjWithPrep() + " " + rNoun(false) + " the last couple days.";
	}
	
	public static String parentNote(String name){
		return "Please make sure " + name + " does not " + rVerb() + " with the " + rAdj() + " " + rNoun() + ".";
	}
